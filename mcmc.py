...
from util import Distribution, is_alpha
import string
import math
import random
import sys
...
def build_letter_transition_dist(file):
	...
	for i in range(1, len(doc)):
		first_letter = doc[i-1] if is_alpha(doc[i-1]) else " "
		second_letter = doc[i] if is_alpha(doc[i]) else " "
...
def clean_document(document):
	...
	return document.translate(translator).lower()
...
def compute_log_likelihood(document, expected_letter_distribution):
	...
	for i in range(1, len(document)):
		first_letter = document[i-1].lower() if is_alpha(document[i-1]) else " "
		second_letter = document[i].lower() if is_alpha(document[i]) else " "
...
def decrypt_document(encrypted_document, cipher):
	...
	for c in encrypted_document:
		if (is_alpha(c) and (c.lower() in mapping)):
...
def propose_cipher_tuneable(current_cipher, beta=1):
	"""
	Choose how many swaps to do according to a tuneable exponential distribution
	"""

	n = 30
	density = list(map(lambda x: math.exp(-beta*x), range(n)))
	s = sum(density)
	density = [x/s for x in density]

	n_swaps = random.choices(population=range(n), weights=density, k=1)[0]

	for i in range(n_swaps):
		current_cipher = propose_cipher(current_cipher)
...
def test_metropolis_hastings(encrypted_document, expected_letter_distribution, 
							correct_document, beta, max_iter=None, metric=None, verbose=True):
	"""
	Tests the Metropolis-Hastings algorithm to decode a known document and known corrected document. 
	"""
	encrypted_document = clean_document(encrypted_document)

	current_cipher = generate_random_cipher()

	# (document content, log likelihood, cipher that generated the doc)
	best_document = ("", float("-inf"), "")
	...
	while(True): 
		i+=1
		proposal_cipher = propose_cipher_tuneable(current_cipher, beta)
		...
		if (log_likelihood_proposal > best_document[1]):
			best_document = (proposal_document, log_likelihood_proposal, proposal_cipher)

		if(acceptance_criteria(log_likelihood_proposal, log_likelihood_current)):
			history.append((proposal_cipher, log_likelihood_proposal))
			number_accepted += 1
			current_cipher = proposal_cipher
		...
		if best_document[0] == correct_document or (max_iter and i>= max_iter):
			break

	if metric is None:
		metric_result = None
	else:
		metric_result = metric(history)

	return best_document, number_accepted, i, metric_result
...
if __name__ == '__main__':
    ...
    # plot_freq(expected_letter_distribution)
    # print(generate_random_cipher())